  ...
  return (
    <>
      <PageHeader
        ...
        description="Real-time monitoring, AI-powered flood prediction and emergency decision support."
        action={<span className="prototype-label">LIVE DATA MONITORING</span>}
      />

      <section className="dashboard-status-banner">
        <div>
          <span className="eyebrow">CURRENT FLOOD STATUS</span>
          <h2>{status}</h2>
          <p>Heavy rainfall and high drainage utilization detected in multiple locations. Immediate monitoring and preparedness actions are recommended.</p>
        </div>
      ...
      <div className="situation-summary-grid">
        <div className="situation-card rainfall">
          ...
          <b>Source: {source.isLive ? 'IMD' : 'IMD fallback / demo'}</b>
        </div>
        <div className="situation-card flood-risk">
          <span>◉</span>
          <small>FLOOD RISK</small>
          <strong>{status}</strong>
          <p>Overall urban flood risk</p>
          <b>{criticalZones} locations require attention</b>
        </div>
        <div className="situation-card drainage">
          <span>≋</span>
          <small>DRAINAGE STATUS</small>
          <strong>{Math.round(prediction.drainage.utilization * 100)}%</strong>
          <p>Average network utilization</p>
          <b>{prediction.drainage.overloadedNodes.length} overloaded nodes</b>
        </div>
        <div className="situation-card roads">
          <span>▣</span>
          <small>ROADS AFFECTED</small>
          <strong>{affectedRoads}</strong>
          <p>Closed or restricted roads</p>
          <b>Emergency access available</b>
        </div>
      </div>
        ...
        <Panel className="dashboard-map-card">
          <div className="panel-heading">
            <div>
              ...
              <p className="muted">3D-style Mumbai terrain, flood hotspots, infrastructure and critical access corridors.</p>
            </div>
          ...
          <div className="dashboard-map-hero">
            ...
            <div className="map-glass map-status-overlay">
              ...
              <small>Mumbai Metropolitan Region · 3D terrain view</small>
            </div>
          ...
          <div className="dashboard-map-footer">
            <span><i className="dash-legend critical" />Critical flood zone</span>
          ...
          <div className="dashboard-alert-list">
            {priorityStreets.map((street, index) => (
              ...
              >
                ...
                <h3>{street.name}</h3>
                <p>Predicted water depth may reach <strong>{street.waterDepth} cm</strong>.</p>
                <small>Action: {street.waterDepth >= 60 ? 'Restrict non-emergency traffic' : street.waterDepth >= 30 ? 'Prepare emergency response' : 'Monitor and limit heavy vehicles'}</small>
                <button
                  type="button"
                  onClick={(event) => {
      ...
      <Panel className="dashboard-forecast-panel">
        <div className="panel-heading">
          <div>
            ...
            <p className="muted">Expected progression based on current rainfall and flood prediction inputs.</p>
          </div>
        ...
        <div className="dashboard-timeline">
          ...
          <div>
            <b>+30 MIN</b>
            <span>Drainage network approaching critical utilization</span>
          </div>
          <div>
            <b>+45 MIN</b>
            <span>Kurla Station Road predicted to experience critical flooding</span>
          </div>
          <div>
            <b>+60 MIN</b>
            <span>Traffic restrictions and emergency access measures recommended</span>
          </div>
        </div>
      </Panel>
      ...
      <Panel className="ai-situation-insight">
        <div>
          <span className="eyebrow">AI SITUATION INSIGHT</span>
          <h2>What is driving the current risk?</h2>
          <p>
            <strong>AI analysis indicates that {insightFactors[0][0].toLowerCase()} and low-lying terrain are the primary contributors</strong> to critical flooding near {priorityStreets[0].name}.
          </p>
        </div>
        ...
        <Link className="text-link" to="/explainable-ai">
          Open explainable analysis <span>→</span>
        </Link>
      </Panel>
      ...
      <section className="quick-modules-section">
        <div className="section-heading">
          <div>
            <span className="eyebrow">DECISION SUPPORT MODULES</span>
            <h2>Quick decision modules</h2>
          </div>
          <span className="muted">Connected system capabilities</span>
        </div>
      ...
      <Panel className="data-sources-panel">
        <div>
          <span className="eyebrow">DATA & SYSTEM STATUS</span>
          <h2>Unified urban flood intelligence inputs</h2>
        </div>
        <div className="data-source-list">
          <span><b>Rainfall Data</b>{source.isLive ? 'IMD' : 'IMD / demo fallback'}</span>
          <span><b>Drainage Network</b>Municipal infrastructure data</span>
          <span><b>Terrain Data</b>DEM / elevation model</span>
          <span><b>Flood Prediction</b>AI-assisted flood model</span>
          <span><b>Route Analysis</b>Road network and flood-risk engine</span>
        </div>
      ...
      <footer className="dashboard-footer">
        ...
        <span>Prototype developed for Smart India Hackathon 2026</span>
        <small>Data sources and integrations: IMD Rainfall • Municipal Infrastructure • Terrain / DEM • AI Flood Prediction</small>
      </footer>
    </>
  )
}
